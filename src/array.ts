 ...
 *
 */
Array.prototype.reduceMap = function reduceMap(
  callback: (cur: any, index: number, array: any[]) => any,
) {
...
Array.prototype.where = function (predicate: (item: any) => boolean) {
  const result: any[] = []
  for (let i = 0; i < (<Array<any>>this).length; i++) {
    const item: any = (<Array<any>>this)[i]
    if (predicate(item)) {
      result.push(item)
    }
  }
...
Array.prototype.orderByDescending = function (
  propertyExpression: (item: any) => any,
) {
  const result = []
  const compareFunction = (item1: any, item2: any): number => {
    if (propertyExpression(item1) > propertyExpression(item2)) return -1
...
Array.prototype.orderByMany = function (
  propertyExpressions: [(item: any) => any],
) {
  const result = []
  const compareFunction = (item1: any, item2: any): number => {
...
Array.prototype.orderByManyDescending = function (
  propertyExpressions: [(item: any) => any],
) {
  const result = []
  const compareFunction = (item1: any, item2: any): number => {
